...
class RZPZ_Henvis {
    ...
    public static function handle_save_form_config() {
        ...
        // Core fields are always enabled + required
    ...
    public static function handle_save_smtp() {
        ...
        // Don't overwrite password if left blank (keep existing)
        if ( empty( $smtp['pass'] ) ) {
    ...
    private static function process_form( $post, $cfg ) {
        ...
        $emails_log = self::send_emails( $referrer_name, $referrer_email, $friend_name, $friend_email, $manager );
    ...
    private static function send_emails( $referrer_name, $referrer_email, $friend_name, $friend_email, $manager ) : array {
        ...
        // Headers shared across all emails (CC = extra recipients)
        ...
        $sent1 = wp_mail(
            $manager['email'],
            sprintf( '🙌 %s har henvist en ven til Rezponz!', $referrer_name ),
            self::email_wrap( "
                <p>Hej {$manager['name']}!</p>
                <p>Gode nyheder – <strong>{$referrer_name}</strong> har netop henvist <strong>{$friend_name}</strong> til at søge en stilling hos Rezponz. 💪</p>
                <p>Kontaktoplysninger på vennen:</p>
                <ul>
                  <li><strong>Navn:</strong> {$friend_name}</li>
                  <li><strong>Email:</strong> <a href='mailto:{$friend_email}' style='color:#CCFF00'>{$friend_email}</a></li>
                </ul>
                <p><strong>{$friend_name}</strong> har fået en invitationsmail. Din opgave er at følge op og motivere! 🚀</p>
        ...
        $sent2 = wp_mail(
            ...
            self::email_wrap( "
                <p>Hej {$friend_name}!</p>
                <p>Du er lige blevet henvist af <strong>{$referrer_name}</strong>, som mener du ville passe perfekt hos Rezponz.</p>
                ...
                <p>👉 <a href='https://rezponz.dk/karriere-stillinger/' style='background:#CCFF00;color:#0d0d0d;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:700;display:inline-block;margin:8px 0'>Søg en stilling her</a></p>
        ...
        $sent3 = wp_mail(
            ...
            self::email_wrap( "
                ...
                <p>Vi har nu sendt <strong>{$friend_name}</strong> en invitationsmail, og din Senior Manager <strong>{$manager['name']}</strong> er notificeret.</p>
    ...
    public static function handle_export_csv() {
        ...
        fputcsv( $out, [ 'ID', 'Dato', 'Medarbejder navn', 'Medarbejder email', 'Medarbejder tlf', 'Ven navn', 'Ven email', 'Ven tlf', 'Manager', 'Status', 'Note' ], ';' );
